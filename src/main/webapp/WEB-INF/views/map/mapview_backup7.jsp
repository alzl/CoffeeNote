            ...
            <li class="bookmark-item" onclick="showCafesByBookmark(${bookmark.bookmarkId})">
                ...
                <c:choose>
                    <c:when test="${not empty loggedInUserId}">
                        <!-- 좋아요 버튼 (로그인한 사용자일 경우) -->
                        <img src="<c:out value='${bookmark.liked ? "/images/like.png" : "/images/unlike.png"}'/>" 
                             class="like-icon" 
                ...
                좋아요: <span class="like-count">${bookmark.likes}</span>
            </li>
        </c:forEach>
    </ul>
</div>
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.error) {
                ...
                $(element).data('liked', response.isLiked);  // jQuery data() 메서드로 저장
                $(element).attr('data-liked', response.isLiked);  // HTML 속성으로 설정
